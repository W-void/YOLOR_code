...
(
    select userid
        ...
        ,concat_ws('\'','\'', collect_list(cur_poi_bid_list)) as cur_poi_bid_list
        ,concat_ws('\'','\'', collect_list(cur_poi_charge_rate_list)) as cur_poi_charge_rate_list
        ...
        ,concat_ws('\'','\'', collect_list(imp_list)) as imp_list
    ...
    (
        select userid
            ...
            ,case when pageid = '\''cpsEverDayCoupon'\'' then PaddingOperator(cur_poi_bid_list, $len_EverDayCoupon)
                else PaddingOperator(cur_poi_bid_list, $len_SelfCoupon)
                end as cur_poi_bid_list
            ,case when pageid = '\''cpsEverDayCoupon'\'' then PaddingOperator(cur_poi_charge_rate_list, $len_EverDayCoupon)
                else PaddingOperator(cur_poi_charge_rate_list, $len_SelfCoupon)
                end as cur_poi_charge_rate_list
            ...
            ,case when pageid = '\''cpsEverDayCoupon'\'' then PaddingOperator(imp_list, $len_EverDayCoupon)
                else PaddingOperator(imp_list, $len_SelfCoupon)
                end as imp_list
...
\"
=== 
targetTable==${targetTable}_step2
=== 
dropTable==False
"
...
# nohup sh task_parallel.sh 20240701 20240817 part1_feature_extract_step2.sh &